...
diverso, per il quale **l'algoritmo greedy potrebbe non produrre la
...
// Funzione che divide l'array e restituisce l'indice del pivot
// Funzione che divide l'array e restituisce l'indice del pivot
int partition(int arr[], int low, int high) {
    int pivot = arr[high]; // scegliamo l'ultimo elemento come pivot
    int i = low - 1; // indice del più grande elemento

    for (int j = low; j <= high - 1; j++) {
        // se l'elemento corrente è maggiore o uguale al pivot
        if (arr[j] >= pivot) {
            i++; // incrementiamo l'indice del più grande elemento
            swap(&arr[i], &arr[j]); // scambiamo arr[i] e arr[j]
        }
    }
    swap(&arr[i + 1], &arr[high]); // scambiamo arr[i+1] e arr[high] (o il pivot)
...
void quickSort(int arr[], int low, int high) {
    if (low < high) {
        // pi è l'indice del pivot dopo la partizione
        int pi = partition(arr, low, high);

        // Ricorsivamente ordiniamo gli elementi prima e dopo la partizione
...
int resto(int R, int m[], int n) {
    unsigned int numM = 0;
    int i = 0;
    // Ordina l'array m[] utilizzando QuickSort
    quickSort(m, 0, n - 1);
