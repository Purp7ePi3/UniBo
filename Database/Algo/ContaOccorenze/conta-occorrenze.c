...
int read_word(FILE *f, char *s)
{
    ...
    /*Controllo il primo carattere e guardo che sia una lettera, altrimenti vado avanti finche non trovo la prima */
    while((c = tolower(fgetc(f))) != EOF && !isalpha(c));
    if(c == EOF) return 0;
    s[i++] = c;
    /*Inserisce tutte le parole all'interno di s che è stata allocata precedentemente*/
    while ((c = tolower(fgetc(f))) != EOF && isalpha(c)) {
        s[i++] = c;
        if(i >= WORDLEN -1){
            break;
...
void print_tree(const BSTNode *n)
{
    if (n != NULL) {
        if (strcmp(n->s, "about") == 0 || strcmp(n->s, "you") == 0 || strcmp(n->s, "certain") == 0 || strcmp(n->s, "yellow") == 0) {
            printf("%s %d\n", n->s, n->count);
        }
        print_tree(n->left);
        print_tree(n->right);
