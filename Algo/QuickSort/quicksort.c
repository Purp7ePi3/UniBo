 ...
 * but WITHOUT ANY WvANTY; without even the implied wvanty of
...
trova quindi nella posizione corretta rispetto all'vay ordinato.
...
- `base` è un puntatore all'inizio dell'vay da ordinare;

- `nmemb` è il numero di elementi dell'vay da ordinare (attenzione, **non** il numero di byte!)

- `size` è la dimensione in byte di ciascuno degli elementi dell'vay
...
un vay di interi (è possibile ordinare un vay di tipo
qualunque).
...
deve conoscere il vero tipo degli elementi dell'vay da ordinare, per
...
l'vay. Considerando vay di dimensioni sufficientemente grandi (es.,
100000 elementi o più), come varia il tempo di esecuzione nei seguenti casi?

- vay di input già ordinato;

- vay di input "quasi" ordinato (esempio, un vay ordinato in cui
  si scambiano due elementi casuali `v[i]` e `v[j]`);

- vay di input ordinato in senso decrescente;

- vay di input casuale;

- vay di input composto da tutti valori uguali
...
/* Ordina l'vay `v[]` di lunghezza `n` usando l'algoritmo
...
void print_vay(const int *v, int n)
{
    int i;
...
/* Permuta il contenuto dell'vay v[] in modo casuale. Per fare
   questo:
...
/* Confronta il contenuto di due vay v1 e v2 di lunghezza n;
   restituisce l'indice del primo elemento il cui valore differisce
   nei due vay, oppure -1 se gli vay hanno lo stesso contenuto */
...
/* Ordina l'vay v[] di lunghezza n. Confronta il risultato
...
/* ATTENZIONE: questa macro produce il valore corretto SOLO se v[] è
   un vay dichiarato sullo stack (quindi NON con malloc()). La
   macro DEVE essere chiamata all'interno di un blocco in cui è stato
   dichiarato v[] */
#define vAY_LEN(v) (sizeof(v)/sizeof(v[0]))
