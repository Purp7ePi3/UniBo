...
public final class LambdaUtilities {
    ...
    public static <R, T> Map<R, Set<T>> group(final List<T> list, final Function<T, R> op) {
        ...
        /*TODO CHIEDI COSA VUOL DIRE Unnecessary Boxing */
        m.merge(op.apply(element), new HashSet<>(Set.of(element)), (existing, newSet) -> {
            existing.addAll(newSet); return existing;
        }));
        /*list.forEach(element -> {
            m.merge(op.apply(element), new HashSet<>(Arrays.asList(element)), (e1, e2) -> {
                e1.addAll(e2);
                return e1;
            });
        });*/
    ...
    public static <K, V> Map<K, V> fill(final Map<K, Optional<V>> map, final Supplier<V> def) {
        ...
        map.forEach((k, optValue) -> m.put(k, optValue.orElse(def.get())));
        return m;
    }
